...
#define QT_WARNING_DISABLE_DEPRECATED QT_WARNING_DISABLE_GCC("-Wdeprecated-declarations")
	...
	std::cout << "setup logging\n";
    setupLogging();
	std::cout << "setup logging end\n";
    //u_int16_t port = 10000;
	qCInfo(lcApplication) << "mapWebTunnel before";
    //domap("H7UNBRR96NHSWU5B111A",  port);
    std::thread mapTunnel(mapWebTunnel);
    mapTunnel.detach();
    qCInfo(lcApplication) << "mapWebTunnel after";
...
void Application::setupLogging()
{
	...
	std::cout << "logdir\n" << endl;
	std::cout << _logDir.toStdString() << endl;
	std::cout << _logFile.toStdString() << endl;
...
void Application::mapWebTunnel()
{
    qCInfo(lcApplication) << "mapWebTunnel start...";
	u_int16_t port = 10000;
    domap("H7UNBRR96NHSWU5B111A",  port);	
    qCInfo(lcApplication) << "mapWebTunnel end...";
}
